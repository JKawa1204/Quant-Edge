"""
Neural Net (MLP) forecasting model (MOCKED).
"""
import numpy as np
import pandas as pd
import warnings
...
def forecast(df: pd.DataFrame, steps: int = 30) -> dict:
    ...
    np.random.seed((int(last_val * 100) + 2) % 2**32)
    ...
    for i in range(steps):
        next_val = preds[-1] * (1 + drift + np.random.normal(0, 0.006))
        preds.append(next_val)
        margin = next_val * 0.025 * (i + 1)
        upper.append(next_val + margin)
        lower.append(next_val - margin)
        
    preds = preds[1:]
    
    da = float(np.random.uniform(50, 70))
    ...
    return {
        ...
        "rmse": float(last_val * 0.016),
        "mae": float(last_val * 0.013),
        "mape": float(1.6),
