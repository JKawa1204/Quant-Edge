...
export function getStockDetail(symbol: string) {
  ...
  const seededRand = (hash * 12345 % 100) / 100;
  const mcap = 50_000_000_000 + seededRand * 19_500_000_000_000; // 50K Cr to 20L Cr
  ...
  return {
    ...
    marketCap: mcap,
    eps: Math.round(10 + seededRand * 90),
    pe: Math.round((10 + seededRand * 50) * 100) / 100,
    bookValue: Math.round(price * (0.2 + seededRand * 0.6)),
    dividendYield: Math.round((0.5 + seededRand * 4) * 100) / 100,
    roe: Math.round(5 + seededRand * 25),
    roa: Math.round(2 + seededRand * 15),
    debtToEquity: Math.round(seededRand * 2 * 100) / 100,
    revenue: Math.round(mcap * (0.1 + seededRand * 0.4)),
    netProfit: Math.round(mcap * (0.01 + seededRand * 0.05)),
    operatingMargin: Math.round(10 + seededRand * 20),
    freeCashFlow: Math.round(mcap * (0.005 + seededRand * 0.03)),
    week52High: price * (1.05 + seededRand * 0.3),
    week52Low: price * (0.7 + seededRand * 0.25),
    beta: Math.round((0.5 + seededRand * 1.0) * 100) / 100,
    volatility: Math.round(10 + seededRand * 20),
    avgVolume: Math.round(500_000 + seededRand * 9_500_000),
